...
const StudentProfile = () => {
  ...
  const { data: userData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure(
        `https://summer-camp-server-one.vercel.app/studentProfile/${user?.email}`
      );
      console.log(res);
      return res.data;
    },
  ...
  const onSubmit = (data) => {
    ...
    axiosSecure.put(`https://summer-camp-server-one.vercel.app/studentProfile/${userData?._id}`,data)
    .then(updateData => {
    ...
    <div className="w-full p-5 md:p-14">
      ...
      <div>
        <div>
          <div className="max-w-full relative mx-auto md:w-1/2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="flex px-10 gap-6 items-center pb-10">
              ...
              <div>
                ...
                <button onClick={() => setShow(!show)} className="rounded-full top-0 right-5 bg-[#75D5E3] absolute">
                  <FaRegEdit></FaRegEdit>
                </button>
            </div>
          </div>
          ...
          <animated.div   style={{...zoomIn}} className={`card mt-12 w-full ${show ? "opacity-100" :  "opacity-0"} border flex-1 shadow-2xl ${Dark ? "text-white" : "text-slate-800"}`}>
            <div className="card-body">
              <h1 className="text-3xl text-center mb-3 font-bold">Update Profile</h1>
